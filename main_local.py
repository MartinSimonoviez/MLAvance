...
if args.st != 0 and args.dataset == 'multi': # pas là que ça charge mes données
...
if args.net_resume:
    ...
    net.load_state_dict(
        torch.load(p1, map_location=torch.device('cpu'))
    )
    twin.load_state_dict(
        torch.load(p2, map_location=torch.device('cpu'))
    )
else:
    net.load_state_dict(
...
lr = args.lr
# Instead of net.cuda() and twin.cuda()
